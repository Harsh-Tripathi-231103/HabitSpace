...
router.get("/summary", async (req, res) => {
    ...
    const habitStatuses = habits.map((habit) => evaluateHabitChallenge({
        startDate: habit.startDate,
        checkIns: habit.checkIns.map((c) => c.checkDate),
    }));
    const trackerStatuses = trackerTasks.map((task) => evaluateTrackerTask({
    ...
    const habitSummary = {
        ...
        averageStreak: habitStatuses.length === 0
            ? 0
