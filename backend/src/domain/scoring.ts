...
export type Status = "WIN" | "LOSE" | "IN_PROGRESS";

export function evaluateHabitChallenge(input: {
  startDate: Date;
  checkIns: Date[];
  now?: Date;
}): { streak: number; status: Status; dayNumber: number } {
...
export function cadenceWindow(cadence: TrackerCadence, dueDate: Date): { windowStart: Date; windowEnd: Date } {
  if (cadence === "DAILY") {
...
export function evaluateTrackerTask(input: {
  ...
  completions: Date[];
  now?: Date;
}): { status: Status; windowStart: Date; windowEnd: Date } {
  const now = input.now ?? new Date();
